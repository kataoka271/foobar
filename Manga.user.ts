...
`);
  ...
  class Slides {
    ...
    pagenum: HTMLDivElement;
    seeker: HTMLDivElement;
    seekbar: HTMLDivElement;
    slides: HTMLDivElement;
    numPages: number;
    slideOffset: number;
    wheelHoldTimeStamp: number;
    imageList: WrappedImage[];
  ...
  const imgs = $$<HTMLImageElement>(site.imgs);
  if (imgs.length == 0) {
    return;
  }
